    ...
    >
      <Container>
        <Nav className={`me-auto my-2 my-lg-0`}>
          {navItems["hydra:member"].slice(0, maxElements).map((el) => (
            <Fragment key={el.id}>
              <Nav.Item className="me-3">
                ...
                >
                  {el.items ? (
                    el.items.map((item) => (
                      <Fragment key={item.id}>
                        {!item.subItems.length ? (
                          <MenuItem key={item.id} className="pb-1 pt-1">
                            <Link
                              to={`/shop/category/${
                    ...
                    <MenuItem key={el.id} className="fs-14 text-dark">
                      {" "}
                      <Link
        ...
        <div className="d-flex flex-row">
          <div className="me-3">
            <Link to="/shop/type/coffrets">
              <img src="/img/coffret.png" className="img-fluid" alt="coffret" />
            </Link>
          </div>
